...
Viewport::drawAxis()
{
    ...
    glLineWidth( 1.5f );
    glBegin( GL_LINES ); {
        glColor3fv( glm::value_ptr(Tool::getAxialColor(0)) );
        glVertex3f( c.x, c.y, c.z );
        glVertex3f( x.x, x.y, x.z );
        glColor3fv( glm::value_ptr(Tool::getAxialColor(1)) );
        glVertex3f( c.x, c.y, c.z );
        glVertex3f( y.x, y.y, y.z );
        glColor3fv( glm::value_ptr(Tool::getAxialColor(2)) );
        glVertex3f( c.x, c.y, c.z );
...
void
Viewport::loadPickMatrices( const QRectF& rect ) const
{
    const glm::mat4 &modelview = m_camera->getModelviewMatrix();
    const glm::mat4 &projection = m_camera->getProjectionMatrix();
    ...
    float width = (float)m_width;
    float height = (float)m_height;
    float tX = 2.f*(width/2.f- (rect.left() + rect.right())/2.f )/(rect.right() - rect.left());
    float tY = 2.f*((rect.top() + rect.bottom())/2.f - height/2.f+1.f)/(rect.bottom() - rect.top());
    const glm::mat4 translate = glm::translate( glm::mat4(1.f), glm::vec3(tX,tY,0.f) );
    glMultMatrixf( glm::value_ptr(translate) );
    const glm::mat4 scale = glm::scale( glm::mat4(1.f), glm::vec3(width/(rect.right() - rect.left()), height/(rect.bottom() - rect.top()), 1.f) );
    glMultMatrixf( glm::value_ptr(scale) );
    glMultMatrixf( glm::value_ptr(projection) );
